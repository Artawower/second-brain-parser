...
describe("Tools test", () => {
  ...
  it("Should normalize org text", () => {
    expect(normalizeStringValue("   some text with BIG WORDs   ")).toEqual(
      "some text with big words"
    );
  });

  it("Should not change normal string after normalization", () => {
    expect(normalizeStringValue("text should not be normalized")).toEqual(
      "text should not be normalized"
    );
  });
...
describe("File name container uuid checker", () => {
  it("Should contain uuid4", () => {
    ...
    expect(isFileNameContainUuid(`./long-path/my-file${uuid()}.png`)).toBe(
      true
    );
    expect(isFileNameContainUuid(`./long-path/my-file${uuid()}.webp`)).toBe(
      true
    );
    expect(
      isFileNameContainUuid(
        `./long-path/my-file${uuid()}another-info.not-ext.txt`
      )
    ).toBe(true);
    expect(
      isFileNameContainUuid(`./long-path/longer/another.file-${uuid()}`)
    ).toBe(true);
  });
  ...
  it("Should not contain uuid4", () => {
    ...
    expect(isFileNameContainUuid(`./long-path/my-file.not-ext.txt`)).toBe(
      false
    );
    expect(isFileNameContainUuid(`./long-path/longer/another.file`)).toBe(
      false
    );
